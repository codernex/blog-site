...
const Sidebar = ({ title, menu }) => {
    ...
    return (
        <>
            ...
            <aside className="sidebar dark:bg-dark_color dark:text-white !z-20 ">
                <div className="title dark:text-white text-dark_color">
                    {title}
                </div>
                <ul className="menu">
                    {menu.map((item, index) => (
                        <li key={index}>
                            {item.menu && <div className="nav-menu dark:text-white text-dark_color">{item.menu}</div>}
                            {item?.label && !item.child && (
                                <Link href={item.href || '#!'} className="nav-link dark:text-white text-dark_color"
                                    onClick={() => {
                                        window.document.querySelector('.sidebar').classList.toggle('open')
                                        window.document.querySelector('.sidebar-overlay').classList.toggle('open')
                                    }}

...
const Table = ({
    ...
    const checkPermissions = (name) => {
        if (permission) {
            return havePermission(name, role);
        }
        return true;
    };
                                    ...
                                    <>
                                        {(pagination ? data?.docs : data)?.map((row, index) => (
                                            <tr key={index}>
                                                ...
                                                {cols?.map((column, index) => (
                                                    <td
                                                        className={`whitespace-nowrap p-2 text-gray-500 ${
                                                            column?.className || ''
                                                        }`}
                                                        key={index}
                                                    >
        ...
        <div className='w-inline-block h-8'>
            ...
            <Modal
                ...
                closeIcon={
                    <FaTimes
                        size={18}
                        className='absolute right-4 top-4 inline-block rounded bg-gray-300 bg-opacity-25 text-white'
                    />
                }
            >
